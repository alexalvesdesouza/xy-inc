...
public class PontoInteresseServico {
  ...
  /**
   * Serviço responsável por fazer o registro de um ponto de interesse visitado.
   * 
   * @param entity
   * @return
   */
  ...
  /**
   * Serviço responsável por retornar uma lista com todos os pontos de interesse.
   * 
   * @return
   */
  ...
  /**
   * Serviço responsável por retornar todos os pontos de interesse próximos de uma determinada
   * localização.
   * 
   * @param coordenadaX
   * @param coordenadaY
   ...
   * @return
   */
  public Collection<PontoInteresse> listaPontosInteresseProximos(Integer coordenadaX, Integer coordenadaY, Double distancia) {
    List<PontoInteresse> pontosProximos = new ArrayList<>();

    this.listaTodosPontosInteresse()
        .iterator()
        .forEachRemaining(poi -> {
  ...
  private Boolean isPontoInteresseEProximo(Integer pontoReferenciaX, Integer pontoReferenciaY, Double distancia, PontoInteresse poi) {

    Double resultadoLatitudesX = calculaDistanciaLatitudesLongitudes(poi.getCoordenadaX()
                                                                        .doubleValue(),
                                                                     pontoReferenciaX.doubleValue());
    Double resultadoLongitudeY = calculaDistanciaLatitudesLongitudes(poi.getCoordenadaY()
                                                                        .doubleValue(),
                                                                     pontoReferenciaY.doubleValue());

    BigDecimal perimetro = new BigDecimal(distancia).setScale(10);

    return new BigDecimal(determinaPontosProximosPlanoCartesiano(resultadoLatitudesX, resultadoLongitudeY)).setScale(10, RoundingMode.HALF_EVEN)
                                                                                                           .compareTo(perimetro) <= BigDecimal.ZERO.intValue();
